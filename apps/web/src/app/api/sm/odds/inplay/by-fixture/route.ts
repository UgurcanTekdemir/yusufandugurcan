...
/**
 * GET /api/sm/odds/inplay/by-fixture?fixtureId=123&bookmakerId=23&include=markets:name;bookmakers:name&filters=markets:1,14
 * Fetch in-play odds for a fixture from a specific bookmaker
 * 
 * Query parameters:
 ...
 * 
 * Static filters:
 * - markets:12,14 - Filter by market IDs (comma-separated)
 * 
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await sportmonksClient.getInplayOddsByFixtureAndBookmaker(
      fixtureId,
      bookmakerId,
      {
    ...
    // Handle different response formats (same as prematch/by-fixture)
    ...
    if (response && typeof response === "object" && "data" in response) {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (firstItem.fixture_id && firstItem.market_id && firstItem.label && firstItem.value) {
          console.log("[INPLAY ODDS BY BOOKMAKER] Detected flat odds array format, transforming to markets format");
          ...
          console.log("[INPLAY ODDS BY BOOKMAKER] Transformed flat odds array to markets format:", {
        ...
        if (dataObj.odds && typeof dataObj.odds === "object") {
          ...
          if (Array.isArray(odds)) {
            console.log("[INPLAY ODDS BY BOOKMAKER] Detected odds array format, transforming to markets format");
      ...
      if (firstItem.fixture_id && firstItem.market_id && firstItem.label && firstItem.value) {
        console.log("[INPLAY ODDS BY BOOKMAKER] Detected flat odds array format (direct array), transforming to markets format");
    ...
    console.error("Error fetching inplay odds by fixture and bookmaker:", error);
