...
/**
 * GET /api/sm/squads/seasons/[seasonId]/teams/[teamId]?include=player;details.type
 * Fetch squad (player statistics) for a team in a specific season
 * Based on SportMonks API v3 documentation
 */
export async function GET(
  request: NextRequest,
  { params }: { params: { seasonId: string; teamId: string } }
) {
  try {
    ...
    const include = query.include || "player;details.type";

    const response = await sportmonksClient.getSquadBySeasonAndTeam(
    ...
    // Return the squad data (array of player statistics)
    ...
    if (error instanceof z.ZodError) {
      ...
      const isRouteError = error.errors.some(
        (err) =>
          (err.path.length === 1 && err.path[0] === "seasonId") ||
          (err.path.length === 1 && err.path[0] === "teamId")
      );

      if (isRouteError) {
        return NextResponse.json(
          { error: "Invalid season ID or team ID", details: error.errors },
          { status: 400 }
        );
      } else {
